...
sub _build_vr_sample
{
  ...
  #( $self->use_supplier_name && defined $self->supplier_name ) ? $self->accession : $self->name;
...
sub _populate_individual
{
  ...
  #~ if( defined($self->supplier_name)) 
  #~ {
    #~ $vr_individual->alias($self->supplier_name);
  #~ }
  #~ if( defined($self->cohort_name)) 
  #~ {
    #~ $vr_individual->acc($self->cohort_name);
  #~ }
  #~ if( defined($self->control)) 
  #~ {
    #~ $self->control == 1 ? $vr_individual->sex('M') : $vr_individual->sex('F');
  #~ }  
